...
export const getUserHandler: RouteHandler<{
  ...
  if (findUser) {
    ...
    reply.code(200).send({
      success: true,
      message: "User found",
      user: { email_address: email, state: state as UserState, ...user },
    });
    ...
    if (method === "Bearer") {
      ...
      if (findToken?.token === token) {
        ...
        reply.send({
          ...
          user: { state: state as UserState, ...user },
        });
      } else {
...
export const updateUserHandler: RouteHandler<{
  ...
  condData = !email_address
    ? { ...condData }
    : { ...condData, email: email_address };
  condData = !hash_password
    ? { ...condData }
    : { ...condData, hash_password: hash_password };
  condData = !state
    ? { ...condData }
    : { ...condData, state: state as UserState };
  ...
  if (updateUser) {
    ...
    reply.code(200).send({
      ...
      user: { state: state as UserState, ...sendUser },
    });
  } else {
    ...
    if (findUser.hash_password) {
      ...
      if (verifyPassword) {
        reply.send({
          ...
          user: { role: findUser.role, id: findUser.id },
        });
      } else {
